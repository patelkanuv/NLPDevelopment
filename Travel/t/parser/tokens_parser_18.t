...
my $token1  = Travel::Air::Search::Tokens::Parser->new(
                data_str    => 'Family trip to Los Angeles and Disneyland in August',
                'IP'        => '113.20.16.231'
            );
$token1->parse_input;
...
cmp_ok($token1->return_airport->[0]->airport_code, 'eq', 'LAX', '0_to_airport_code_check');

cmp_ok($token1->trip_class, 'eq', 'Economy', 'trip_class_check');
cmp_ok($token1->trip_type, 'eq', 'RoundTrip', 'trip_type_check');

cmp_ok($token1->depart_date, 'eq', '2013/08/01', 'depart_date_check');
...
cmp_ok($query->to_airport->[0]->airport_code, 'eq', 'LAX', '0_to_airport_code_check');

cmp_ok($query->trip_class, 'eq', 'Economy', 'trip_class_check');
cmp_ok($query->trip_type, 'eq', 'RoundTrip', 'trip_type_check');

cmp_ok($query->depart_date, 'eq', '2013/08/01', 'depart_date_check');
cmp_ok($query->return_date, 'eq', '2013/08/04', 'return_date_check');
