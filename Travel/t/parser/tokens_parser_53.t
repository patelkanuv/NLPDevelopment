...
my $token1  = Travel::Air::Search::Tokens::Parser->new(
                data_str    => 'Ahmadabad to Melbourne next Thursday for 4 days',
                'IP'        => '113.20.16.231'
            );
$token1->parse_input;

my $date_parser = Utility::Date::Simple->new();
my $date1       = $date_parser->get_date_from_sentence('next Thursday');
my $date2       = $date_parser->add_days_to_date($date1, 4);
...
cmp_ok($token1->return_airport->[0]->airport_code, 'eq', 'MLB', '0_to_airport_code_check');
...
cmp_ok($query->to_airport->[0]->airport_code, 'eq', 'MLB', '0_to_airport_code_check');
