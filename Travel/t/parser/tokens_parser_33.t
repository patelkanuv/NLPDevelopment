...
my $token1  = Travel::Air::Search::Tokens::Parser->new(
                data_str    => 'bucharest and transylvania halloween 2013',
                'IP'        => '113.20.16.231'
            );
$token1->parse_input;
...
cmp_ok($token1->return_airport->[0]->airport_code, 'eq', 'OTP', '0_to_airport_code_check');

cmp_ok($token1->trip_class, 'eq', 'Economy', 'trip_class_check');
cmp_ok($token1->trip_type, 'eq', 'OneWay', 'trip_type_check');

cmp_ok($token1->depart_date, 'eq', '2013/10/30', 'depart_date_check');
...
cmp_ok($query->to_airport->[0]->airport_code, 'eq', 'OTP', '0_to_airport_code_check');
