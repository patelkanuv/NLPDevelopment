...
my $token1  = Travel::Air::Search::Tokens::Parser->new(
                data_str    => 'tickets to jamaica for party of six',
                'IP'        => '113.20.16.231'
            );
$token1->parse_input;
...
cmp_ok($token1->return_airport->[0]->airport_code, 'eq', 'KIN', '0_to_airport_code_check');
...
cmp_ok($token1->pax_adult, '==', 6, 'adult_check');
...
cmp_ok($query->to_airport->[0]->airport_code, 'eq', 'KIN', '0_to_airport_code_check');
...
cmp_ok($query->adult, '==', 6, 'adult_check');
