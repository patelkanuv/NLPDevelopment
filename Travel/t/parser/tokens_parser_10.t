...
my $token1  = Travel::Air::Search::Tokens::Parser->new(
                data_str    => 'family trip to San Diego zoo next month',
                'IP'        => '113.20.16.231'
            );
$token1->parse_input;

my $date_parser = Utility::Date::Simple->new();
my $date1       = $date_parser->get_date_from_sentence('next month');
my $date2       = $date_parser->add_days_to_date($date1, 3);
...
cmp_ok($token1->return_airport->[0]->airport_code, 'eq', 'SAN', '0_to_airport_code_check');
...
cmp_ok($token1->pax_child, '==', 2, 'child_check');
...
cmp_ok($query->to_airport->[0]->airport_code, 'eq', 'SAN', '0_to_airport_code_check');
...
cmp_ok($query->return_date, 'eq', '2013/06/04', 'return_date_check');

cmp_ok($query->adult, '==', 2, 'adult_check');
cmp_ok($query->child, '==', 2, 'child_check');
