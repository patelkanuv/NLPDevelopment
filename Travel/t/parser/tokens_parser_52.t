...
my $token1  = Travel::Air::Search::Tokens::Parser->new(
                data_str    => 'Ahmadabad to Australia for 10 to 15 days',
                'IP'        => '113.20.16.231'
            );
$token1->parse_input;

my $date_parser = Utility::Date::Simple->new();
my $date1       = $date_parser->get_next_day_from_today(15);
my $date2       = $date_parser->get_next_day_from_today(15 + 12);
...
cmp_ok($token1->return_airport->[0]->airport_code, 'eq', 'CBR', '0_to_airport_code_check');
...
cmp_ok($query->to_airport->[0]->airport_code, 'eq', 'CBR', '0_to_airport_code_check');
