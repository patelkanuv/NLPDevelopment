...
my $token1  = Travel::Air::Search::Tokens::Parser->new(
                data_str    => 'what are the rates for travel to italy in the summer',
                'IP'        => '113.20.16.231'
            );
$token1->parse_input;
...
cmp_ok($token1->return_airport->[0]->airport_code, 'eq', 'FCO', '0_to_airport_code_check');
...
cmp_ok($query->to_airport->[0]->airport_code, 'eq', 'FCO', '0_to_airport_code_check');
