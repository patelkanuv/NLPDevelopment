...
my $token1  = Travel::Air::Search::Tokens::Parser->new(
                data_str    => 'lowest fares edmonton to paris around summer ',
                'IP'        => '113.20.16.231'
            );
$token1->parse_input;
...
cmp_ok(scalar(@{$token1->depart_airport}), '==', 2, 'from_airport_check');

cmp_ok($token1->depart_airport->[0]->airport_code, 'eq', 'YEG', '0_from_airport_code_check');

cmp_ok(ref($token1->return_airport), 'eq', 'ARRAY', 'to_airport_check');
cmp_ok(scalar(@{$token1->return_airport}), '==', 4, 'to_airport_check');

cmp_ok($token1->return_airport->[0]->airport_code, 'eq', 'ORY', '0_to_airport_code_check');

cmp_ok($token1->trip_class, 'eq', 'Economy', 'trip_class_check');
cmp_ok($token1->trip_type, 'eq', 'OneWay', 'trip_type_check');

cmp_ok($token1->depart_date, 'eq', '2013/06/01', 'depart_date_check');
...
cmp_ok(scalar(@{$query->from_airport}), '==', 2, 'from_airport_check');

cmp_ok($query->from_airport->[0]->airport_code, 'eq', 'YEG', '0_from_airport_code_check');

cmp_ok(ref($query->to_airport), 'eq', 'ARRAY', 'to_airport_check');
cmp_ok(scalar(@{$query->to_airport}), '==', 4, 'to_airport_check');

cmp_ok($query->to_airport->[0]->airport_code, 'eq', 'ORY', '0_to_airport_code_check');

cmp_ok($query->trip_class, 'eq', 'Economy', 'trip_class_check');
cmp_ok($query->trip_type, 'eq', 'OneWay', 'trip_type_check');

cmp_ok($query->depart_date, 'eq', '2013/06/01', 'depart_date_check');
