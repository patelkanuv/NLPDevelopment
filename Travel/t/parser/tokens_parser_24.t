...
my $token1  = Travel::Air::Search::Tokens::Parser->new(
                data_str    => 'flights vancouver tokyo sept 1 to 14',
                'IP'        => '113.20.16.231'
            );
$token1->parse_input;
...
cmp_ok($token1->return_airport->[0]->airport_code, 'eq', 'HND', '0_to_airport_code_check');
...
cmp_ok($token1->return_date, 'eq', '2013/09/14', 'return_date_check');
...
cmp_ok($query->to_airport->[0]->airport_code, 'eq', 'HND', '0_to_airport_code_check');
...
cmp_ok($query->return_date, 'eq', '2013/09/14', 'return_date_check');
