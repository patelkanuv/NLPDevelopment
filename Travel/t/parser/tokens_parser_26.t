...
my $token1  = Travel::Air::Search::Tokens::Parser->new(
                data_str    => 'lancaster pa for three days starting march 14',
                'IP'        => '113.20.16.231'
            );
$token1->parse_input;
...
cmp_ok($token1->depart_date, 'eq', '2014/03/14', 'depart_date_check');
cmp_ok($token1->return_date, 'eq', '2014/03/17', 'return_date_check');
...
cmp_ok($query->depart_date, 'eq', '2014/03/14', 'depart_date_check');
cmp_ok($query->return_date, 'eq', '2014/03/17', 'return_date_check');
