...
my $token1  = Travel::Air::Search::Tokens::Parser->new(
                data_str    => 'i would like to spend two weeks in miami this winter ',
                'IP'        => '113.20.16.231'
            );
$token1->parse_input;
...
cmp_ok($token1->return_airport->[0]->airport_code, 'eq', 'MIA', '0_to_airport_code_check');
...
cmp_ok($token1->return_date, 'eq', '2013/12/15', 'return_date_check');
...
cmp_ok($query->to_airport->[0]->airport_code, 'eq', 'MIA', '0_to_airport_code_check');
...
cmp_ok($query->return_date, 'eq', '2013/12/15', 'return_date_check');
