...
my $token1  = Travel::Air::Search::Tokens::Parser->new(
                data_str    => 'cost of flights toronto nyc june 21 return july 1',
                'IP'        => '113.20.16.231'
            );
$token1->parse_input;
...
cmp_ok($token1->depart_date, 'eq', '2013/06/21', 'depart_date_check');
cmp_ok($token1->return_date, 'eq', '2013/07/01', 'return_date_check');
...
cmp_ok($query->depart_date, 'eq', '2013/06/21', 'depart_date_check');
cmp_ok($query->return_date, 'eq', '2013/07/01', 'return_date_check');
