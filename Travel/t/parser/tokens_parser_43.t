...
my $token1  = Travel::Air::Search::Tokens::Parser->new(
                data_str    => 'one way ticket to moscow july 7 ',
                'IP'        => '113.20.16.231'
            );
$token1->parse_input;
...
cmp_ok($token1->return_airport->[0]->airport_code, 'eq', 'DME', '0_to_airport_code_check');

cmp_ok($token1->trip_class, 'eq', 'Economy', 'trip_class_check');
cmp_ok($token1->trip_type, 'eq', 'OneWay', 'trip_type_check');

cmp_ok($token1->depart_date, 'eq', '2013/07/07', 'depart_date_check');
...
cmp_ok($query->to_airport->[0]->airport_code, 'eq', 'DME', '0_to_airport_code_check');

cmp_ok($query->trip_class, 'eq', 'Economy', 'trip_class_check');
cmp_ok($query->trip_type, 'eq', 'OneWay', 'trip_type_check');

cmp_ok($query->depart_date, 'eq', '2013/07/07', 'depart_date_check');
