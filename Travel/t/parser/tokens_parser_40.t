...
cmp_ok($token1->depart_airport->[0]->airport_code, 'eq', 'MNP', '0_from_airport_code_check');

cmp_ok(ref($token1->return_airport), 'eq', 'ARRAY', 'to_airport_check');
cmp_ok(scalar(@{$token1->return_airport}), '==', 1, 'to_airport_check');

cmp_ok($token1->return_airport->[0]->airport_code, 'eq', 'NYC', '0_to_airport_code_check');

cmp_ok($token1->trip_class, 'eq', 'Economy', 'trip_class_check');
cmp_ok($token1->trip_type, 'eq', 'RoundTrip', 'trip_type_check');

cmp_ok($token1->depart_date, 'eq', '2014/02/18', 'depart_date_check');
cmp_ok($token1->return_date, 'eq', '2014/02/21', 'return_date_check');
...
cmp_ok($query->from_airport->[0]->airport_code, 'eq', 'MNP', '0_from_airport_code_check');

cmp_ok(ref($query->to_airport), 'eq', 'ARRAY', 'to_airport_check');
cmp_ok(scalar(@{$query->to_airport}), '==', 1, 'to_airport_check');

cmp_ok($query->to_airport->[0]->airport_code, 'eq', 'NYC', '0_to_airport_code_check');

cmp_ok($query->trip_class, 'eq', 'Economy', 'trip_class_check');
cmp_ok($query->trip_type, 'eq', 'RoundTrip', 'trip_type_check');

cmp_ok($query->depart_date, 'eq', '2014/02/18', 'depart_date_check');
cmp_ok($query->return_date, 'eq', '2014/02/21', 'return_date_check');
